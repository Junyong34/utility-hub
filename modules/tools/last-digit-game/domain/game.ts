...
export interface StopLastDigitGameAttemptResult {
...
export function startLastDigitGameAttempt(
  state: LastDigitGameState
): LastDigitGameState {
  if (!canStart(state)) {
    return state;
  ...
  const nextIndex =
    state.phase === 'readyToNext'
      ? state.currentUserIndex + 1
      : state.currentUserIndex;
  const selected = state.users[nextIndex];
  ...
  return {
    ...
    isRunning: true,
    phase:
      state.phase === 'readyToSecondStart'
        ? 'readyToSecondStart'
        : 'readyToFirstStop',
  };
}

function unchangedStopResult(
  state: LastDigitGameState
): StopLastDigitGameAttemptResult {
  return {
    state,
...
export function stopLastDigitGameAttempt(
  state: LastDigitGameState,
  formattedTime: string,
  finishedAt: number
): StopLastDigitGameAttemptResult {
  if (!canStop(state)) {
    return unchangedStopResult(state);
  }

  const current = state.users[state.currentUserIndex];
  ...
  const previousTop = getTopDoneUserId(state.users);
  const resolvedTimes = copiedTimes.slice(0, 2);
  const score = calculateScore(
    resolvedTimes[0] ?? '00:00:00',
    resolvedTimes[1] ?? '00:00:00'
  );
  const isGameComplete = state.currentUserIndex === state.users.length - 1;
  ...
  return {
    state: {
      ...
      users,
      isRunning: false,
      phase: isGameComplete ? 'result' : 'readyToNext',
    },
    completedUserId: current.id,
    isGameComplete,
    isNewLeader: nextTop !== null && nextTop !== previousTop,
  };
}
