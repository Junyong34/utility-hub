...
import { FormFieldGroup, FormSectionGroup } from '@/shared/ui/FormSectionGroup';
import {
  Select,
  SelectContent,
  SelectItem,
...
import { DSR_DEFINITION_REFERENCE, DSR_POLICY_OPTIONS } from '../ui.ts';
import { DsrResultCard } from './DsrResultCard.tsx';
import { LoanEditorCard } from './LoanEditorCard.tsx';
import { useDsrCalculator } from './useDsrCalculator.ts';
...
export function DsrCalculatorForm() {
  ...
  const selectedPolicy =
    DSR_POLICY_OPTIONS.find(option => option.value === policyVersion) ??
      ...
      <div className="space-y-8">
        <Card className="border-primary/20 bg-primary/5">
          <CardHeader className="border-b border-primary/10">
            ...
            <CardDescription className="text-sm leading-relaxed text-foreground/80">
              DSR은 연소득 대비 1년 동안 갚아야 하는 모든 대출 원리금의
              비율입니다. 비율이 높을수록 추가 대출 한도는 더 보수적으로
              계산됩니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-3 pt-4 md:flex-row md:items-center md:justify-between">
            <p className="text-sm text-muted-foreground">
              처음 보는 분이라면 공식 정의를 먼저 확인한 뒤 계산기를 사용해
              보세요.
            </p>
            <a
        ...
        <Card>
          <CardHeader className="border-b">
            ...
            <CardDescription>
              연소득과 정책 버전을 정하면 현재 금리 기준 DSR과 스트레스 DSR을
              함께 계산합니다.
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              >
                <Select
                  value={policyVersion}
                  onValueChange={value =>
                    setPolicyVersion(value as typeof policyVersion)
                  }
                >
              ...
              <div className="rounded-lg border border-dashed px-4 py-4">
                ...
                <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
                  실제 심사에서는 금융회사 내부 규정, 정책모기지 예외, 업권별
                  DSR 방식이 추가로 반영될 수 있습니다.
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                대출별 잔액, 금리, 남은 만기, 금리 구조를 입력하면 보유 대출
                연원금과 연이자를 자동 계산합니다.
              </p>
            </div>
          ...
          {existingLoans.length === 0 ? (
            <div className="rounded-xl border border-dashed px-5 py-6 text-sm text-muted-foreground">
              아직 보유 대출을 추가하지 않았습니다. 대출이 없다면 이 상태로
              계산해도 됩니다.
            </div>
          ) : (
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              희망 대출 금액을 입력하면 현재 금리 기준 DSR과 스트레스 DSR을
              비교하고, 가능한 최대 신규 대출 한도도 함께 계산합니다.
            </p>
          </div>
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-relaxed text-amber-950">
          입력한 조건을 기준으로 계산한 예상값입니다. 실제 대출 가능 금액과 적용
          금리, 최종 심사 결과는 금융회사 기준에 따라 달라질 수 있으니 최종
          한도는 금융사에서 확인하세요.
        </div>
