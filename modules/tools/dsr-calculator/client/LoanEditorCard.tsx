    ...
    <CardContent className="space-y-5 pt-4">
        ...
        >
          <Select
            ...
            onValueChange={value =>
              onChange({
                ...loan,
                repaymentMethod: value as DsrLoanInput['repaymentMethod'],
              })
            }
          >
            <SelectTrigger className="h-10 w-full">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {loan.loanType === 'mortgage' ? (
          ...
          <div className="rounded-lg border border-dashed px-3 py-3 text-sm text-muted-foreground">
            신용대출은 지역 구분 없이 신용대출 합산 잔액 기준으로 스트레스 DSR을
            판단합니다.
          </div>
        )}
      </div>
      ...
      {loan.rateType === 'mixed' || loan.rateType === 'periodic' ? (
        <div className="space-y-2">
          <Label
            htmlFor={`${loan.id}-introductory-period`}
          ...
          <p className="text-xs text-muted-foreground">
            금리유형별 스트레스 DSR 비율은 이 기간이 전체 만기에서 차지하는
            비중으로 계산합니다.
          </p>
        </div>
      ) : null}
