...
import type { DsrCalculationResult } from '../public.ts';
import { currencyFormatter } from '../ui.ts';
      ...
      <CardHeader className="border-b">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              현재 금리 기준 DSR과 스트레스 DSR, 가능한 신규 대출 한도를 함께
              보여줍니다.
            </CardDescription>
          </div>
          ...
          <div className="rounded-xl border bg-card px-4 py-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {formatCurrencyToKoreanUnits(
                result.maxLoanEstimate.maxLoanAmount
              )}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-3">
                      <div>
                        ...
                        <p className="font-semibold">
                          {currencyFormatter.format(loan.annualPrincipal)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="font-semibold">
                          {currencyFormatter.format(loan.annualInterest)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
          ...
          <div className="space-y-3 rounded-xl border bg-card p-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                실제 금리 기준과 스트레스 금리 기준의 연간 원리금 차이를
                비교합니다.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-lg border border-dashed px-3 py-3">
                ...
                <p className="mt-1 font-semibold">
                  {currencyFormatter.format(
                    result.currentNewLoanDebtService.annualPrincipal
                  )}
                </p>
                ...
                <p className="mt-1 font-semibold">
                  {currencyFormatter.format(
                    result.currentNewLoanDebtService.annualInterest
                  )}
                </p>
              </div>
              <div className="rounded-lg border border-dashed px-3 py-3">
                ...
                <p className="mt-1 font-semibold">
                  {currencyFormatter.format(
                    result.stressedNewLoanDebtService.annualPrincipal
                  )}
                </p>
                <p className="mt-2 text-xs text-muted-foreground">
                  스트레스 금리 기준 연이자 상환액
                ...
                <p className="mt-1 font-semibold">
                  {currencyFormatter.format(
                    result.stressedNewLoanDebtService.annualInterest
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">
                  +{result.policy.appliedStressRate}%p
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">적용 사유</span>
                <span className="font-semibold text-right">
                  {result.policy.stressRateReason}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">연간 원리금 여유</span>
                <span
                  className={`font-semibold ${result.remainingCapacity >= 0 ? 'text-primary' : 'text-destructive'}`}
                >
        ...
        <div className="rounded-xl border bg-card p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold">
                {result.policy.baseStressRate}%
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">단계 반영 비율</span>
              <span className="font-semibold">
                {Math.round(result.policy.stageFactor * 100)}%
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">금리유형 반영 비율</span>
              <span className="font-semibold">
                {Math.round(result.policy.rateTypeFactor * 100)}%
              </span>
            </div>
          </div>
          <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
            ...
            <li>
              - 실제 금융회사 심사에서는 상품 약관, 정책모기지 예외, 자행대환
              조건 등에 따라 결과가 달라질 수 있습니다.
            </li>
          </ul>
        </div>
      </CardContent>
