...
import type { HomeBuyingInput, HomeBuyingResult } from '../../public';
...
import { SpendingFlowChart } from '../../ui';
...
export function HomeBuyingResultSection({
  result,
  currentCash,
  input,
}: HomeBuyingResultSectionProps) {
  // 계산된 값들
  const downPayment = Math.floor(
    input.salePrice * (input.downPaymentRatio / 100)
  );
  const balance =
    result.breakdown.find(item => item.id === 'balance')?.amount ?? 0;
  ...
  const totalTaxes = result.breakdown
    .filter(
      item =>
        item.category === 'public' || item.category === 'loan-registration'
    )
