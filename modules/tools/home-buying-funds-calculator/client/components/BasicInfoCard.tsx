...
import { calculateDefenseFundAmount } from '../../public';
...
export function BasicInfoCard({ input, onChange }: BasicInfoCardProps) {
  ...
  const actualLoanAmount = input.hasDefenseFund
    ? input.loanAmount - defenseFundAmount
    : input.loanAmount;
      ...
      <CardContent className="space-y-6">
        ...
        <AmountInputField
          ...
          quickActions={QUICK_ACTIONS.salePrice.map(action => ({
            ...
            label: action.label,
            onSelect: () =>
              onChange({ salePrice: input.salePrice + action.amount }),
          }))}
        />
        ...
        <div className="space-y-2">
          <AmountInputField
            ...
            quickActions={QUICK_ACTIONS.loanAmount.map(action => ({
              ...
              label: action.label,
              onSelect: () =>
                onChange({ loanAmount: input.loanAmount + action.amount }),
            }))}
          />
          <p className="text-xs text-muted-foreground">
            * 인지세는 대출금액에 따라 자동 계산됩니다 (1천만원~5천만원: 5만원,
            5천만원~1억: 10만원, 1억 초과: 15만원)
          </p>
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={checked =>
                  onChange({ hasDefenseFund: !!checked })
                }
              />
              <Label
                htmlFor="hasDefenseFund"
            ...
            <p className="text-xs text-muted-foreground pl-6">
              방공제는 은행이 소액임차인 보호를 위해 대출금에서 차감하는
              금액입니다.
              <br />
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem
                      value="overconcentration"
                      id="regional-overconcentration-basic"
                    />
                    <Label
                      htmlFor="regional-overconcentration-basic"
                      className="cursor-pointer font-normal text-sm"
                    >
                      과밀억제권역 (5,000만원)
                    </Label>
              ...
              <AmountInputField
                ...
                onChange={value =>
                  handleManualAmountChange('manualDefenseFundAmount', value)
                }
              ...
              <p className="text-sm text-muted-foreground">
                실제 대출 가능 금액: {actualLoanAmount.toLocaleString('ko-KR')}
                원
                <br />
                <span className="text-xs">
                  (대출금 {input.loanAmount.toLocaleString('ko-KR')}원 - 방공제{' '}
                  {defenseFundAmount.toLocaleString('ko-KR')}원)
                </span>
              </p>
            </>
          )}
        ...
        <div className="space-y-2">
          <AmountInputField
            ...
            quickActions={QUICK_ACTIONS.currentCash.map(action => ({
              ...
              label: action.label,
              onSelect: () =>
                onChange({ currentCash: input.currentCash + action.amount }),
            }))}
          />
          ...
          <div className="flex items-center gap-2">
            <Checkbox
              ...
              onCheckedChange={checked =>
                onChange({ hasDownPaymentPaid: !!checked })
              }
            />
            <Label
              htmlFor="hasDownPaymentPaid"
              className="cursor-pointer font-normal"
            >
            ...
            <Tooltip>
              ...
              <TooltipContent side="right">
                <p className="max-w-xs whitespace-pre-line">
                  {FIELD_TOOLTIPS.hasDownPaymentPaid}
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              onChange={e =>
                handlePercentChange('downPaymentRatio', e.target.value)
              }
              placeholder="10"
