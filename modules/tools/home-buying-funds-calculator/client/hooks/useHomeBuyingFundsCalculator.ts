...
export function useHomeBuyingFundsCalculator() {
  ...
  const input: HomeBuyingInput = useMemo(
    () => ({
      ...
      regionalType: state.regionalType as
        | 'seoul'
        | 'overconcentration'
        | 'metro'
        | 'other',
      ...
      cleaningFeePreset: state.cleaningFeePreset as
        | 'none'
        | 'basic'
        | 'premium'
        | 'manual',
      movingFeePreset: state.movingFeePreset as
        | 'small'
        | 'medium'
        | 'large'
        | 'manual',
  ...
  const result: HomeBuyingResult = useMemo(
    () => calculateHomeBuyingFunds(input),
    [input]
  );
