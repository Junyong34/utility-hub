...
test('취득세와 인지세는 지방세법 제11조제1항제8호 기준 구간 경계 결과를 갖는다', () => {
  // 2026-08-03 기준 「지방세법」 제11조제1항제8호(law.go.kr, MST=282559, JO=001100) 검증 결과,
  // 기존 취득세 구간(6천만원 breakpoint, 1.3%/2.8%/4% 임의 기울기)이 법정 세율표와 달라
  // 아래 기대값을 실제 법령 기준(6억 이하 1%, 6~9억 보간식, 9억 초과 3%)으로 갱신했다.
  //
  // 옛 코드에는 6천만원 지점에 별도 breakpoint가 있었지만 새 세율표(가목: 6억 이하 전부 1%
  // 단일 구간)에는 그런 경계가 없으므로 60,000,000/60,000,001 쌍은 더 이상 "경계값 테스트"로서
  // 의미가 없어 제거했다. 6억~9억 보간식(나목)의 내부 값·반올림 tie 케이스는
  // `standard-acquisition-tax-rate.test.mjs`에서 손으로 유도한 값으로 별도 검증한다.
  assert.deepEqual(
    {
      acquisitionTax: [600_000_000, 600_000_001, 900_000_000, 900_000_001].map(
        salePrice => [
          salePrice,
    ...
    {
      acquisitionTax: [
        [600_000_000, 6_000_000],
        [600_000_001, 6_000_000],
        [900_000_000, 27_000_000],
        [900_000_001, 27_000_000],
      ],
      stampTax: [
        [0, 0],
        [10_000_000, 0],
