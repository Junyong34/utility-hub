...
test('무주택 생애최초 매수자가 8억 주택을 사면 감면이 적용된다', () => {
  ...
  //
  // 손으로 유도한 기대값(「지방세법」 제11조제1항제8호 나목):
  //   raw = (800,000,000×2/300,000,000 − 3) = 5.3333... − 3 = 2.3333...
  //   → 세율 = 2.3333...%, 반올림(넷째자리) → 2.33% (5번째 소수자리 3이라 내림)
  //   → 산출세액 = 800,000,000 × 0.0233 = 18,640,000원 (감면 미적용 시 이 값 그대로)
  //   → 생애최초 감면(제36조의3제1항제2호): 산출세액 200만원 초과 → 산출세액 − 200만원
  //     = 18,640,000 − 2,000,000 = 16,640,000원
  ...
  assert.equal(withoutRelief, 18_640_000);
  assert.equal(withRelief, 16_640_000);
});
