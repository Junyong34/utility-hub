...
import type { CostBreakdownItem } from '../../public';
...
export function SpendingFlowChart({
  ...
  const flowItems = STAGE_ORDER.map(stage => stageGroups[stage]).filter(
    Boolean
  );

  // 계약금 지불 완료 시 총 비용에서 제외
  const downPaymentAmount =
    breakdown.find(item => item.id === 'down-payment')?.amount ?? 0;
  const totalAmount = breakdown.reduce((sum, item) => sum + item.amount, 0);
  const displayTotalAmount = hasDownPaymentPaid
      ...
      <CardContent className="pt-2">
        <div className="relative">
          {flowItems.map((item, index) => {
            ...
            const circleColor =
              STAGE_COLORS[item.stage] ?? 'bg-primary text-primary-foreground';
            const connectorColor =
              STAGE_CONNECTOR_COLORS[item.stage] ?? 'border-border';
        ...
        <div className="mt-4 pt-4 border-t border-border">
          ...
          {hasDownPaymentPaid && downPaymentAmount > 0 && (
            <p className="text-xs text-muted-foreground mt-2 text-right">
              * 계약금({downPaymentAmount.toLocaleString('ko-KR')}원)은 이미
              지불 완료
            </p>
          )}
        </div>
