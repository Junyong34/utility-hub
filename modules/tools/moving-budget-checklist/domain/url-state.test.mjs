...
test('자산, 템플릿 항목, 체크 상태, 추가 항목은 query segment round-trip이 가능하다', () => {
  ...
  state.checklistProgress = ['apartmentPrice', 'cleaning'];
  state.customItems = [
    {
      id: 'custom-1',
  ...
  assert.deepEqual(
    {
      assets: serializeAssetsForQuery(state.assets),
      checklist: serializeChecklistProgressForQuery(state.checklistProgress),
      customItems: serializeCustomItemsForQuery(state.customItems),
      templateItems: serializeTemplateItemsForQuery(state.templateItems),
    },
    {
      assets: '{"c":10000000,"st":0,"dp":0,"sv":0,"ln":90000000}',
      checklist: '["ap","cl"]',
      customItems:
        '[["custom-1","pm","줄눈 시공",900000],["custom-2","mi","커튼",1200000]]',
      templateItems: '[["ap",450000000],["cl",350000]]',
    }
  );

  assert.deepEqual(
    parseAssetsFromQuery(serializeAssetsForQuery(state.assets)),
    state.assets
  );
  assert.deepEqual(
    parseTemplateItemsFromQuery(
