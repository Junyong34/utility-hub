import type { MovingBudgetAssetId, MovingBudgetGroupId } from '../public';

interface MovingBudgetAssetField {
  id: MovingBudgetAssetId;
  label: string;
  description: string;
}

interface MovingBudgetGroupDisplayMetadata {
  accentClass: string;
  bgClass: string;
  textClass: string;
  barClass: string;
...
export const MOVING_BUDGET_ASSET_FIELDS: MovingBudgetAssetField[] = [
  ...
  { id: 'savings', label: '적금', description: '만기 예정 적금' },
  {
    id: 'loan',
    label: '대출 예정금',
    description: '이사에 투입할 대출 예정 금액',
  },
];

export const MOVING_BUDGET_GROUP_META: Record<
  MovingBudgetGroupId,
  MovingBudgetGroupDisplayMetadata
> = {
  'purchase-costs': {
