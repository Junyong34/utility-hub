...
interface LoanInputFormProps {
  ...
  onTermModeChange: (value: 'year' | 'month') => void;
  onTermValueChange: (value: string) => void;
  onMethodChange: (value: RepaymentMethod) => void;
  onReset: () => void;
  onCalculate: () => void;
  addToPrincipal: (amount: number) => void;
  addToRate: (amount: number) => void;
  addToTerm: (amount: number) => void;
}
                ...
                <div className="flex gap-2 items-center">
                  <div className="flex-1">
                    <Input
                      ...
                      value={termValue}
                      onChange={event => onTermValueChange(event.target.value)}
                      placeholder={
                        termMode
                          ? '상환 기간을 입력하세요'
                          : '년/월 단위를 먼저 선택하세요'
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {REPAYMENT_METHODS.map(m => (
                <button
                  key={m.value}
                  onClick={() => onMethodChange(m.value as RepaymentMethod)}
