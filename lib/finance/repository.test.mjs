    ...
    snapshots: [
      {
        ...
        expenses: createDefaultExpenseRows().map(row =>
          row.id === 'fixed-housing' ? { ...row, amount: 800000 } : row
        ),
      },
      {
        month: '2026-03',
        ...
        investments: [],
        expenses: createDefaultExpenseRows().map(row =>
          row.id === 'fixed-housing' ? { ...row, amount: 820000 } : row
        ),
      },
    ],
  });
...
test('여러 스냅샷을 한 번에 저장하면 모든 월이 반영되고 정렬된다', async () => {
  const { repository, filePath } = await createRepoFixture({
    version: 1,
    snapshots: [
      {
  ...
  const savedSnapshots = await repository.saveSnapshots([
    {
      month: '2026-06',
      updatedAt: '2026-06-30T00:00:00.000Z',
      incomes: { husbandSalary: 6000000, wifeSalary: 4200000, memo: '' },
      assets: [],
      debts: [],
      investments: [],
      expenses: createDefaultExpenseRows(),
    },
    {
      ...
      incomes: { husbandSalary: 5500000, wifeSalary: 4000000, memo: '' },
      assets: [],
      debts: [],
      investments: [],
      expenses: createDefaultExpenseRows(),
  ...
  assert.deepEqual(
    savedSnapshots.map(snapshot => snapshot.month),
    ['2026-03', '2026-06']
  );
  assert.deepEqual(
