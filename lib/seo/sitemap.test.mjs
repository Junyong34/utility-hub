...
test('sitemap place pagination URLs do not include filters', async () => {
  ...
  assert.equal(
    paginationUrls.some(url =>
      /[?&](search|age|category|theme|season|indoor|outdoor|free|feeding|stroller|rain|limit|region)=/.test(
        url
      )
    ),
    false
...
test('tool sitemap entries use updatedAt over publishedAt when present', async () => {
  const { collectToolEntries } = await import('./sitemap.ts');
  const { getAllToolConfigs } =
    await import('../../modules/tools/catalog/public.ts');

  const tools = getAllToolConfigs();
  const entries = collectToolEntries();

  const toolWithUpdatedAt = tools.find(tool => tool.updatedAt);
  const toolWithoutUpdatedAt = tools.find(tool => !tool.updatedAt);

  assert.ok(toolWithUpdatedAt, 'expected at least one tool with updatedAt set');
  assert.ok(
    toolWithoutUpdatedAt,
    'expected at least one tool without updatedAt set'
  );

  const updatedEntry = entries.find(entry =>
    entry.url.endsWith(`/tools/${toolWithUpdatedAt.id}`)
  );
  const publishedOnlyEntry = entries.find(entry =>
    entry.url.endsWith(`/tools/${toolWithoutUpdatedAt.id}`)
  );

  assert.ok(updatedEntry);
  assert.equal(updatedEntry.lastModified, toolWithUpdatedAt.updatedAt);

  assert.ok(publishedOnlyEntry);
  assert.equal(
    publishedOnlyEntry.lastModified,
    toolWithoutUpdatedAt.publishedAt
  );
});
