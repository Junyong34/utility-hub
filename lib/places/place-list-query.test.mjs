...
const PLACE_FIXTURES = [
  ...
  },
  {
    ...
    name: '경기남부 야외 무료',
    description: '서울 실내 무료와 비슷한 조건 설명',
...
test('queryPlaceListFrom returns only animal-theme places when the animal theme is selected', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');
  ...
  assert.equal(result.total, 1);
  assert.equal(result.filters.theme, 'animal');
});
...
test('queryPlaceListFrom filters publishable places by season', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');

  const summerResult = queryPlaceListFrom(PLACE_FIXTURES, {
    season: 'summer',
  });
  const allSeasonResult = queryPlaceListFrom(PLACE_FIXTURES, {
    season: 'all-season',
  });

  assert.deepEqual(
    summerResult.places.map(place => place.id),
    ['south-outdoor-free']
  );
  assert.deepEqual(
    allSeasonResult.places.map(place => place.id),
    [
      'seoul-indoor-free',
      'seoul-both-paid-all-age',
      'north-indoor-partial-free',
    ]
  );
});
...
test('queryPlaceListFrom ignores unknown filter values instead of throwing', async () => {
  ...
  assert.equal(result.total, 4);
  assert.equal(result.places.length, 4);
  assert.equal(result.filters.theme, null);
});
...
test('queryPlaceListFrom filters publishable places by title search only', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');
  ...
  assert.equal(result.total, 1);
  assert.equal(result.filters.search, '서울 실내 무료');
});

test('queryPlaceListFrom applies title search within region and category filters', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');
...
test('queryPlaceListFrom treats blank title search as no search filter', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');
  ...
  assert.equal(result.scopeTotal, 4);
  assert.equal(result.total, 4);
  assert.equal(result.filters.search, null);
  assert.deepEqual(
    result.places.map(place => place.id),
    [
      'seoul-indoor-free',
...
test('queryPlaceListFrom normalizes long and non-string title search inputs safely', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');

  const longSearch = `${'서울'.repeat(50)}실내`;
  const longResult = queryPlaceListFrom(PLACE_FIXTURES, {
    search: longSearch,
  });
  const numericResult = queryPlaceListFrom(PLACE_FIXTURES, {
    search: 123,
  });

  assert.equal(longResult.filters.search.length, 80);
  assert.equal(longResult.total, 0);
  assert.equal(numericResult.filters.search, '123');
  assert.equal(numericResult.total, 0);
});

test('queryPlaceListFrom keeps pagination metadata consistent for title search results', async () => {
  const { queryPlaceListFrom } = await import('./place-list-query.ts');
