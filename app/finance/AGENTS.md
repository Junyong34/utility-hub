...
- 순수 계산 변경 시 관련 source-colocated 테스트를, 저장소·파일 I/O 변경 시 `pnpm test:integration:finance`를 실행한다.
- 화면 흐름 변경 시 격리된 설정을 사용하는 `pnpm test:e2e:finance`를 실행한다.
- finance Playwright 테스트는 `FINANCE_SNAPSHOTS_PATH`로 주입된 OS 임시 파일만 사용하며 실제 `data/private/finance-snapshots.json`을 읽거나 백업·복원하지 않는다.
