...
test.describe('/finance detail pages', () => {
  ...
  test('자산, 부채, 투자, 리포트 화면은 카드와 차트로 구성된다', async ({
    ...
    await page.goto('/finance/assets?month=2026-07', {
    ...
    await page.goto('/finance/debts?month=2026-07', {
      waitUntil: 'networkidle',
    });
    ...
    await expect(
      page.getByTestId('finance-investments-composition')
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.getByTestId('finance-expenses-composition')
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.getByTestId('finance-reports-half').locator('svg')
    ).toHaveCount(1);
    await expect(
