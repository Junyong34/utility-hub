...
test.describe('/tools/home-check', () => {
  ...
  test('자산과 비용 입력을 바꾸면 요약 카드와 진행률이 즉시 갱신된다', async ({
    ...
    await expect(page.getByLabel('현금', { exact: true })).toHaveValue(
      '50,000,000'
    );
    await expect(page.getByLabel('주식계좌', { exact: true })).toHaveValue(
      '10,000,000'
    );
    await expect(page.getByLabel('아파트 가격', { exact: true })).toHaveValue(
      '450,000,000'
    );
    await expect(page.getByLabel('입주청소', { exact: true })).toHaveValue(
      '350,000'
    );
